...
def initialize(context):
    ...
    context.slippage = FixedSlippage(spread=0.00, commission=0.00)
...
def EOD(context,data): 
    ...
    if context.open_orders:
        for orders,_ in context.open_orders.items():
            cancel_order(orders)
    context.empty=True
    print("EOD")
    #context.eod = True
...
def select_securities(context, data):
    ...
    context.rolling_volatility = OrderedDict(sorted(list(context.rolling_volatility.items()), key=lambda kv: kv[1], reverse=True))
    print((context.rolling_volatility))
    
    context.securities = {}
    below_limit = 0
    for i in range(context.num_securities):
        underlying = list(context.rolling_volatility.keys())[i]
...
def handle_data(context,data):
    ...
    if context.empty==False:
        for security in list(context.securities.keys()):
            context.bull = context.securities[security]['bull']
            context.bear = context.securities[security]['bear']
            context.underlying = security
